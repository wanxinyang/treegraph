...
#                                 graph_threshold=.05
    ...
    if base_correction:
        ...
        if 'downsample' in pc.columns:
            ...
            low_slice = pc[pc.z <= (min(stem.z)+low_slice_len)]
        # if 'downsample' in pc.columns:
        #     pc_2m = pc[(pc.downsample) & (pc.z < (pc.z.min()+2))][['x','y','z','downsample']]
        #     stem = pd.merge(stem, pc_2m, how='inner')
        # low_slice = stem[(stem.z <= (min(stem.z)+low_slice_len))]
...
def identify_stem(pc, plot=False):
    ...
    # print(f'fitted radius = {r_fit:.2f} m')
    # print(f'fit error = {fit_err:.2f}')
...
#     # identify part of stem points
#     stem_h = identify_stem_part(pc)
#     if math.isnan(stem_h):  # cannot find vertical stem
#         base_h = 1.  # fit a cylinder to 1m at base
        
#         # calculate basel radius and basel centre
#         # but basal radius is not reliable, so set to nan
#         _, _, _, basal_r, new_base = base_fitting(pc, base_slice_len=base_h)
#         basal_r = np.nan
    
#     else:  # can find vertical stem
#         if stem_h > 2: 
#             base_h = 2.  # fit a cylinder to 2m at base
#         else: 
#             base_h = stem_h  # fit a cylinder to stem_h at base
...
#     return pc[columns], G, new_base, basal_r
...
#     Inputs:
#         - pc: dataframe with columns ['x','y','z']
#             Stem point clouds.
...
#     return base_slice, fit_C, axis_dir, r_fit, new_base    
...
# def identify_stem_part(pc):
#     # rough tree height
#     h = pc.z.max() - pc.z.min()
#     print(f'rough tree height = {h:.2f} m')
#     # create empty df for later stem point collection
#     stem = pd.DataFrame(columns=['x','y','z'])
    
#     if h > 20:  # regards as a tall tree
#         step = h / 30.
#         # loop over slices of point clouds with vertical interval of 'step'
#         # loop stop at the 1/10 of the tree height
#         for i in np.arange(0, h/10., step):
#             zmin = pc.z.min()
#             pc_slice = pc[pc.z < (zmin+i+step)]  
#             pc_coor = pc_slice.reset_index()[['x','y','z']]
...
#             # cluster the sliced points
#             dbscan = DBSCAN(eps=.1, min_samples=20).fit(pc_coor)
#             pc_coor.loc[:, 'clstr'] = dbscan.labels_
...
#             #     stem_h = i + step
#             # else:
#             #     if i == 0: # great normal difference at 1st slice
#             #         stem_h = np.nan
#             #         print(f'do not find a good base slice')
#             #     break

#     else:  # for a short tree
#         step = .5
...
#             # #  normal vector of the tree-stem is generally parallel to the ground
#             # nv, d = normal_vector(pc_coor)
#             # if d < 0.5:
#             #     stem_h = i + step
#             # else:
#             #     if i == 0: # great normal difference at 1st slice
...
#             #         print(f'do not find a good base slice')
#             #     break
#     # print(f'height for fitting a cylinder: {stem_h:.2f} m')
#     return stem