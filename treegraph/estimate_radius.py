...
def p2a_displot(pc, centres, path_ids, interval=0.3, branch_list=None, 
                transform_plot=False, displot=False):
    '''
        ...
        if (zmax - zmin) < dz:
            ...
            for i, z in enumerate(np.arange(zmin, zmax+dz, dz)):
                ...
                if displot:
                    ...
                    z_low = z - zmin
                    z_upp = z + dz - zmin
                    ax[0].set_title(f'Slice from {z_low:.2f} to {(z_upp):.2f} m,\np2a_mean={p2a_mean:.2f}, std={section.p2a.std():.2f}, cv={p2a_cv:.2f}',
                                    fontsize=12)
                    # ax[0].set_title(f'section {i}, z=[{z_low:.2f}, {(z_upp):.2f}],\np2a_mean={p2a_mean:.2f}, std={section.p2a.std():.2f}, cv={p2a_cv:.2f}')
                    ...
                    ax[1].axvline(p2a_mean, color='r', label='p2a_mean')
...
# def run(pc, centres, path_ids, zinterval=0.3, branch_list=None, plot=False):
#     '''
#     Radius estimation of individual branches.
#     '''
#     if branch_list is None:
...
#         ## transform branch from curve to straight
#         pc_transf = branch_transform(pc, centres, path_ids, nbranch=nbranch, 
#                                      interp='nearest', plot=False, save=False)
#         if len(pc_transf) == 0: 
#             centres = centres.loc[~(centres.nbranch == nbranch)]
#             continue
#         # calculate point-to-axis distance of each point
...
#         ## smooth taper by moving average
#         taper.loc[:, 'p2a_mean_conv'] = taper.p2a_mean

#         if (len(taper) > 3):
#             taper.loc[taper.index.max()+1, 'dfb'] = zmax
#             taper.loc[taper.index.max(), 'p2a_mean'] = taper.p2a_mean.values[-2]

#             # do not smooth stem radius as stem points usually in good quality
#             if nbranch == 0:
#                 # find stem furcation node
#                 ncyl = centres[centres.ninternode == 0].ncyl.max()
#                 stem_fur_node = centres[(centres.nbranch == 0) & (centres.ncyl == ncyl)].node_id.values[0]
#                 stem_fur_dfb = pc[pc.node_id == stem_fur_node].distance_from_base.max()

#                 window_size = 3
#                 windows = taper[taper.dfb > stem_fur_dfb].p2a_mean.rolling(window_size, min_periods=1)
#                 taper.loc[(taper.dfb > stem_fur_dfb), 'p2a_mean_conv'] = windows.mean()
#             else:
#                 window_size = 3
#                 windows = taper.p2a_mean.rolling(window_size, min_periods=1)
#                 taper['p2a_mean_conv'] = windows.mean()
#                 taper.loc[0:1, 'p2a_mean_conv'] = taper.p2a_mean.values[0:2]
...
#         centres.loc[centres.nbranch == nbranch, 'sf_radius'] = cen_branch.dfb_id.apply(lambda x: taper[taper.index == x].p2a_mean.values[0])                   
#         centres.loc[centres.nbranch == nbranch, 'sf_radius_conv'] = cen_branch.dfb_id.apply(lambda x: taper[taper.index == x].p2a_mean_conv.values[0])

        
#         ## plot sf_radius and sf_radius_conv of changing along distance from base in current branch
#         if plot:
#             branch = centres[centres.nbranch==nbranch]
#             plt.figure(figsize=(14,4))
#             plt.scatter(branch.distance_from_base, branch.sf_radius, s=5, c='r')
#             plt.plot(branch.distance_from_base, branch.sf_radius, label='estimated radius')

#             plt.scatter(branch.distance_from_base, branch.sf_radius_conv, s=5, c='r')
#             plt.plot(branch.distance_from_base, branch.sf_radius_conv, label='smoothed radius')

#             plt.xlabel('Distance from base (m)')
#             plt.ylabel('Radius (m)')
#             plt.title(f'Branch {nbranch}')
#             plt.legend(bbox_to_anchor=(1.2,1))
    
#     return centres


# def branch_transform(pc, centres, path_ids, nbranch=0, interp='slinear', 
#                      plot=False, save=False, treeid=None):
...
#         tt = np.linspace(t.min(), t.max())
#         ax[0].plot(xx(tt), zz(tt), c='b', linewidth=2.5) # spline
#         ax[0].scatter(pc.x, pc.z, s=0.01, c='grey', alpha=0.2) # point cloud
#         # side view
#         ax[1].scatter(y, z, s=10, c='r') # centre nodes
#         tt = np.linspace(t.min(), t.max())
#         ax[1].plot(yy(tt), zz(tt), c='b', linewidth=2.5) # spline
...
# def p2a(pc, centres, path_ids, zinterval=.3, branch_list=None, plot=False):

#     if branch_list is None:
#         branch_list = np.unique(centres.nbranch)
    
#     # loop over each branch
#     # for nbranch in np.unique(centres.nbranch.isin(branch_list)):
#     for nbranch in branch_list:
#         ## transform branch from curve to straight
#         pc_transf = branch_transform(pc, centres, path_ids, nbranch=nbranch, 
#                                      interp='nearest', plot=False, save=False)
...
#                 if len(section) <= 1:
#                     continue
#                 pct95 = np.nanpercentile(section.p2a, 95)  # filter out distance outside 95th percentile
#                 section = section[section.p2a < pct95]
#                 p2a_cv = section.p2a.std() / section.p2a.mean()
#                 taper.loc[i, 'dfb'] = z
#                 if p2a_cv < 1 :
...
# def p2a_displot(pc, centres, path_ids, interval=0.2, 
#                 branch_list=None, transform_plot=False, displot=True):
#     '''
#     Plot distribution of point to axis distance of each section with given interval, 
#     and return the coefficient of variation of each section.

#     Inputs:
#     - pc: pd.DataFrame, point clouds attributes
#     - centres: pd.DataFrame, skeleton nodes attributes
#     - path_ids: dict, list of node_id of each branch path
#     - interval: float, vertical length to segment points into sections
#     - branch_list: list, a list of branch_id for plotting
#     - transform_plot: bool, if True plot branch curve-to-line transformation graphs
#     - displot: bool, if True plot distribution of point to axis distance of each section

#     Outputs:
#     - cv: dict, coefficient of variation of each section
#     - figures if plot is True
#     '''
...
#     for nbranch in samples.nbranch.unique():
#         # transform points of this branch
#         pc_transf = estimate_radius.branch_transform(pc, centres, path_ids, nbranch=nbranch, plot=transform_plot, save=False)
#         # calculate point to axis distance
#         pc_transf.loc[:, 'p2a'] = np.sqrt(pc_transf.x**2 + pc_transf.y**2)

#         # mean p2a dist at each interval as a func of distance from base
#         taper = pd.DataFrame(columns=['dfb', 'p2a_mean'])
        
#         cv[nbranch] = {}
#         for i, z in enumerate(np.arange(pc_transf.z.min(), pc_transf.z.max(), interval)):
#             section = pc_transf[(pc_transf.z >= z) & (pc_transf.z < (z + interval))]
#             if len(section) <= 1:
#                 continue
#             pct95 = np.percentile(section.p2a, 95)  # filter out distance outside 95th percentile
#             section = section[section.p2a < pct95]
#             # taper.loc[i, 'dfb'] = z
#             # taper.loc[i, 'p2a_mean'] = section[section.p2a < pct95].p2a.mean()
            
#             # coefficient of variance
#             cv[nbranch][i] = section.p2a.std() / section.p2a.mean()
            
#             if displot:
#                 section.plot.scatter(x='x', y='y', s=5, c='grey', 
#                                      # xlim=[pc_transf.x.min()/1.05, pc_transf.x.max()*1.05],
#                                      # ylim=[pc_transf.z.min()/1.05, pc_transf.z.max()*1.05],
#                                      title=f'section {i}', fontsize=12)
#                 sns.displot(section, x='p2a', kde=True)
#                 plt.axvline(section.p2a.mean(), color='r', label='p2a.mean')
#                 plt.axvline(np.percentile(section.p2a, 25), color='g', label='25th percentile')
#                 plt.legend()
