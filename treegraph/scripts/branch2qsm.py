...
def run(path, base_idx=None, attribute='nbranch', tip_width=None, verbose=False,
        ...
        txt_file=True, base_corr=False, save_graph=False):
...
base_correction = {base_corr}\nsave_graph = {save_graph}"
    ...
    self.centres = build_skeleton.run(self, eps=.005, verbose=True)
    ...
    self.centres = fit_cylinders.run(self.pc.copy(), self.centres.copy(),
                                     min_pts=self.min_pts, ransac_iterations=5, 
                                     verbose=self.verbose) 
    if txt_file:
        with open(o_f+'.txt', 'a') as f:
            f.write('\n\n----Fit cylinders----')
    ...
    if txt_file:
        with open(o_f+'.txt', 'a') as f:
            ...
            f.write(f'\nBranch height: {round((max(self.cyls.sz) - min(self.cyls.sz)),2)} m')
            f.write(f'\nTotal branch length: {self.cyls.length.sum():.2f} m')
            f.write(f'\nTotal branch volume: {self.cyls.vol.sum():.5f} m³ = {self.cyls.vol.sum()*1e3:.2f} L')
            f.write(f'\nTotal surface area: {self.cyls.surface_area.sum():.2f} ㎡')
