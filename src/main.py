...
def full_tree(wood, slice_interval, min_pts, down_size=0.1, min_cc_dist='auto',
    ...
    for i, (k, v) in enumerate(skeleton_original.iteritems()):
        if len(v) > min_pts:
            ...
            center, rad, error = fit_sphere(wood[v], vec)
...
def small_branch(wood, slice_interval=0.01, min_pts=5, min_cc_dist='auto',
    ...
    for i, (k, v) in enumerate(skeleton_original.iteritems()):
        if len(v) > min_pts:
            ...
            if k in nl:
                if k in nu:
                    vec = np.vstack((skel_coords[nl[i]], skel_coords[i],
                                     skel_coords[nu[i]]))
                else:
                    vec = np.vstack((skel_coords[nl[i]], skel_coords[i]))
            else:
                if k in nu:
                    vec = np.vstack((skel_coords[i], skel_coords[nu[i]]))
                else:
                    vec = skel_coords[i].reshape(1, 3)
