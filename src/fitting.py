...
from third_party.cylinder_fitting import fit
...
#    grid_z = np.arange(np.min(grid_vals[:, 2]) - radius_step,
#                       np.max(grid_vals[:, 2]) + radius_step, radius_step)
#    grid = list(it.product(grid_x, grid_y, grid_z))
